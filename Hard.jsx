//DOWNLOAD FUNCTIONALITY IS CURRENTLY NOT WORKING FOR SOME REASON. I AM WOKING ON IT.
...
function Main() {
  ...
  const handleYTSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5001/api/youtube_converter', {
        method: 'POST',
  ...
  const handleAudioSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5001/api/process_audio", {
        method: "POST",
        body: formData,
      });
      ...
      if (res.ok) {
        setAudioMessage(data.message || "Audio processed successfully!");
        // You can handle the download link if returned by the API
        // For example, you can set a link for downloading the output
        if (data.output_folder) {
          setAudioMessage(prevMessage => `${prevMessage} Download your processed audio from: ${data.output_folder}`);
        }
      } else {
        setAudioMessage(data.error || "Unexpected response.");
      }
    } catch (err) {
